...
public class GroupStatsAPI implements me.infinity.groupstats.api.GroupStatsAPI {
  ...
  public Optional<GroupProfile> getGroupProfile(String group, UUID playerUUID) {
    try {
      return Optional.ofNullable(
          databaseFactory.getGroupProfileFactory().getDaoManagerMap().get(group)
              .queryForId(playerUUID));
  ...
  public boolean saveGroupProfile(String group, GroupProfile groupProfile) {
    try {
      ...
      if (databaseFactory.getGroupProfileFactory().getCache().get(group)
          .containsKey(groupProfile.getUniqueId())) {
        databaseFactory.getGroupProfileFactory().getCache().get(group)
            .replace(groupProfile.getUniqueId(), groupProfile);
      }
    } catch (SQLException e) {
      // problem occurred
      return false;
  ...
  public boolean deleteGroupProfiles(String group) {
    try {
      for (GroupProfile groupProfile : databaseFactory.getGroupProfileFactory().getDaoManagerMap()
          .get(group).queryForAll()) {
