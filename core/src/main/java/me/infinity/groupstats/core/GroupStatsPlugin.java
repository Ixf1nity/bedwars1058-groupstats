...
public final class GroupStatsPlugin extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    this.bedWarsAPI = this.getServer().getServicesManager().getRegistration(BedWars.class)
        .getProvider();
    this.databaseFactory = new DatabaseFactory(this);
    new GroupStatsExpansion().register();

    this.getServer().getServicesManager().register(GroupStatsAPI.class,
        new me.infinity.groupstats.core.GroupStatsAPI(databaseFactory), this, ServicePriority.Low);
  }
  ...
  public void checkDepends(String... plugins) {
    for (String plugin : plugins) {
      ...
      this.getLogger()
          .severe(plugin + " not found, its an important dependency of this plugin. Disabling...");
      this.getLogger().severe("");
